var spatialTemporalGraph = {
	...
	_heatmap: undefined,
	_canvas: undefined,
	_canvasBackground: undefined,
	_canvasAnimation: undefined,
	_messagePoints: undefined,
	...
	_ifMessageColor: false,
	_ifAnimateTrace: false,
		...
		d3.select("#navbar-clear")
			.on("click", function() {
				...
				dataCenter.setGlobalVariable('selectedBaseStation'
												, null
												, self.name);
				d3.select("#BSid-text").text('none');
				self._renderTraceLayer();
			});
		d3.select("#navbar-color")
			.on("click", function() {
				if(self._ifMessageColor === false) {
					self._ifMessageColor = true;
					self._renderTraceLayer();
				}
				else {
					self._ifMessageColor = false;
					self._renderTraceLayer();
				}
			});
		d3.select("#navbar-trace")
			.on("click", function() {
				if(self._ifAnimateTrace === false) {
					if(dataCenter.globalVariables['selectedBaseStation'] != null) {
						self._ifAnimateTrace = true;
						var context = self._canvas.getContext('2d');
						context.clearRect(0, 0, self._canvas.width, self._canvas.height);
						var tmptime = self._sortArrayAnimation(self._messagePoints);
						var minMinutes = tmptime.minMinutes;
						var maxMinutes = tmptime.maxMinutes;
						console.log("render background");
						self._renderAnimateTrace(self._messagePoints, self._canvasBackground, 1440, true);
						var animation = setInterval(runInterval, 200);
						var lineheight = $("#" + self._dayScaleTimelineDivID).height();
						var tmpwidth = $("#" + self._dayScaleTimelineDivID).width();
						var startWidth = minMinutes / 1440 * tmpwidth;
						var endWidth = maxMinutes / 1440 * tmpwidth;
						console.log(lineheight, tmpwidth)
						self._dayTimelineSvgG.append("line")
							.attr("id", "movingline")
							.attr("y1", 0)
							.attr("y2", lineheight)
							.attr("x1", startWidth)
							...
							.transition()
							.ease("linear")
							.duration(200 * (maxMinutes - minMinutes))
							.attr("x1", endWidth)
							.attr("x2", endWidth);
						var minutes = minMinutes;
						console.log("render animation");
						function runInterval() {
							var mark = self._renderAnimateTrace(self._messagePoints, self._canvasAnimation, minutes, false);
							minutes++;
							console.log(minutes)
							if(mark === true) {
					...
					self._ifAnimateTrace = false;
					var context = self._canvasBackground.getContext('2d');
					context.clearRect(0, 0, self._canvas.width, self._canvas.height);
					context = self._canvasAnimation.getContext('2d');
					context.clearRect(0, 0, self._canvas.width, self._canvas.height);
					self._renderTraceLayer();
				}				
			});
		for(var i = 1; i <= 77; i++) {
			d3.select("#selectBS-btn")
				.append("option")
				.attr("value", i.toString())
				.html(i);
		}
		$("#selectBS-btn").change(function() {
			var id = $("#selectBS-btn").val();
			console.log(id);
			dataCenter.setGlobalVariable('selectedBaseStation'
										, id
										, self.name); 
			d3.select("#BSid-text").text(id);
			console.log("change BS to " + id);
			self._renderTraceLayer();
			self.drawAddedRect(id, 
							false,
							false,
	...
	_createAnimateTraceLayer: function(_map) {
		var self = this;
		console.log("create animate trace layer");
		_map.plugin(['AMap.CustomLayer'], function() {
			self._canvasBackground = document.createElement('canvas');
			self._canvasBackground.width = _map.getSize().width;
			self._canvasBackground.height = _map.getSize().height;
			var cus = new AMap.CustomLayer(self._canvasBackground, {
				zooms: [0, 12],
				zIndex: 100
			});
			cus.render = function() {};
		...
		_map.plugin(['AMap.CustomLayer'], function() {
			self._canvasAnimation = document.createElement('canvas');
			self._canvasAnimation.width = _map.getSize().width;
			self._canvasAnimation.height = _map.getSize().height;
			var cus = new AMap.CustomLayer(self._canvasAnimation, {
				zooms: [0, 12],
				zIndex: 100
			});
			cus.render = function() {};
	...
	_renderTraceLayer: function() {
		...
		console.log("now BSid: " + dataCenter.globalVariables['selectedBaseStation']);
		var self = this;
		...
		for(var id in self._dataByDate) {
			...
			self._messagePoints = messagePoints;
	...
	_sortArrayAnimation: function(pointsArray) {
		var self = this;
		var mark = [];
		var newPointsArray = [];
		for(var i = 0; i < pointsArray.length; i++) {
			var e = pointsArray[i];
			var tmp = e.time.split(":");
			var minutes = parseInt(tmp[0], 10) * 60 + parseInt(tmp[1], 10);
			if(mark[minutes] != true) {
				mark[minutes] = true;
				e.time = minutes;
				newPointsArray.push(e);
			}			
		}
		newPointsArray.sort(function(a, b) {
			return a.time - b.time;
		});
		/*for(var i = 0; i < newPointsArray.length - 2; i++) {
			var source = toLngLat(self._map, newPointsArray[i].x, newPointsArray[i].y);
			var target = toLngLat(self._map, newPointsArray[i + 1].x, newPointsArray[i + 1].y);
			self._driving.search([source.lng, source.lat], [target.lng, target.lat]
		...
		self._messagePoints = newPointsArray;
		return {minMinutes: newPointsArray[0].time, 
			maxMinutes: newPointsArray[newPointsArray.length - 1].time};

		function toLngLat(_map, x, y) {
			var LngLat = _map.containTolnglat(new AMap.Pixel(x, y));
	...
	_renderAnimateTrace: function(pointsArray, canvas, maxMinutes, ifBackground) {
		var self = this;
		...
		context.beginPath();
		if(pointsArray[0].time <= maxMinutes) {
			context.moveTo(pointsArray[0].x, pointsArray[0].y);
		}
		for(var i = 1; i < pointsArray.length; i++) {
			if(pointsArray[i].time > maxMinutes) {
				var k = (maxMinutes - pointsArray[i - 1].time) / (pointsArray[i].time - pointsArray[i - 1].time);
				var x = pointsArray[i - 1].x + (pointsArray[i].x - pointsArray[i - 1].x) * k;
				var y = pointsArray[i - 1].y + (pointsArray[i].y - pointsArray[i - 1].y) * k;
				context.lineTo(x, y);
				circleX = x;
				circleY = y;
				break;
		...
		context.strokeStyle = '#8E8E8E';
		context.fillStyle = '#D0D0D0';
		context.arc(circleX, circleY, 3, 0, Math.PI * 2);
		context.fill();
		context.stroke();
		if(maxMinutes > pointsArray[pointsArray.length - 1].time) {
			return true;
		}
		else {
			return false;
	...
	drawAddedRect: function(index, ifType, ifTotal, data, g, divID) {
		...
		addedBars.enter()
			...
			.attr("width", barWidth)
			.attr("height", function(d, i) {
				if(dataToDraw[i] < 0 || !dataToDraw[i]) {
					return 0;
				}
	...
	_requestDataByDate: function(date) {
		...
		//	console.log(data);
			...
			if(self._ifCreatedLayer === false) {
				...
				self._createAnimateTraceLayer(self._map);
