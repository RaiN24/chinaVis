...
var baseStationOverview = {
    ...
    _createTimeMatrix:function(data){
    	var self=this;
    	self.$bsView.remove("svg:first-child");
    	...
    	var days=Object.keys(data).sort(sortDate),
    	    yCount=days.length,
			rectMargin = {top: 12, text: 20, left: 50,right:10,bottom:10},
			mainHeight=self._svgHeight-rectMargin.left-rectMargin.right,
			mainWidth=self._svgWidth-rectMargin.top-rectMargin.bottom;

    	var unitW=unitH=Math.min(mainWidth/24,mainHeight/yCount);
	...
	_getData:function(selectedBaseStation){
		var self=this;
		console.log(selectedBaseStation);
		//遍历该基站对应的所有电话号码，获取数据
	...
	OMListen: function(message, data) {
		...
		if(message === "set:selectedBaseStation") {
			//handle message
			self._getData(dataCenter.globalVariables.selectedBaseStation);
		}
	}
}
